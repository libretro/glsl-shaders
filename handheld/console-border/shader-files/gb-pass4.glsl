...
uniform COMPAT_PRECISION vec2 TextureSize;
uniform COMPAT_PRECISION vec2 InputSize;
uniform COMPAT_PRECISION vec2 ConsoleBorderGameBoyDotMatrixAlphaBlendTextureSize;
uniform COMPAT_PRECISION vec2 ConsoleBorderGameBoyDotMatrixAlphaBlendInputSize;
uniform sampler2D Texture;
uniform sampler2D BACKGROUND;
uniform sampler2D COLOR_PALETTE;
uniform sampler2D ConsoleBorderGameBoyDotMatrixAlphaBlendTexture;
COMPAT_VARYING vec4 TEX0;
COMPAT_VARYING vec2 texel;
...
void main()
{
    ...
    vec2 tex = floor(ConsoleBorderGameBoyDotMatrixAlphaBlendInputSize * active_coord);
    tex = (tex + 0.5) / ConsoleBorderGameBoyDotMatrixAlphaBlendTextureSize;
    
    // Sample all the relevant textures
    vec4 foreground = COMPAT_TEXTURE(ConsoleBorderGameBoyDotMatrixAlphaBlendTexture, tex - screen_offset);
