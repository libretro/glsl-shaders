...
#pragma parameter baseline_alpha "Baseline Alpha" 0.05 -1.0 1.0 0.01
...
uniform float video_scale;
uniform float baseline_alpha;
uniform float response_time;
uniform float grey_balance;
#else
#define video_scale 3.0
#define baseline_alpha 0.05	//analogous to the contrast slider on the original Gameboy, higher values darken the pixels - [-1, 1]  [DEFAULT: 0.05]
#define response_time 0.20	//simulate response time, higher values result in longer color transition periods - [0, 1] [DEFAULT: 0.20]
#define grey_balance 2.6 //useful to fine-tune the balance between the different shades of grey - [2, 4] [DEFAULT: 2.6]
#endif
...
void main()
{
    ...
    _TMP6 = abs(_a0055);
    _input_dummy_rgb = vec3(float((vec3(float(_input_dummy_rgb.x), float(_input_dummy_rgb.y), float(_input_dummy_rgb.z)) + (_TMP6 - vec3(float(_input_dummy_rgb.x), float(_input_dummy_rgb.y), float(_input_dummy_rgb.z)))*response_time).x), float((vec3(float(_input_dummy_rgb.x), float(_input_dummy_rgb.y), float(_input_dummy_rgb.z)) + (_TMP6 - vec3(float(_input_dummy_rgb.x), float(_input_dummy_rgb.y), float(_input_dummy_rgb.z)))*response_time).y), float((vec3(float(_input_dummy_rgb.x), float(_input_dummy_rgb.y), float(_input_dummy_rgb.z)) + (_TMP6 - vec3(float(_input_dummy_rgb.x), float(_input_dummy_rgb.y), float(_input_dummy_rgb.z)))*response_time).z));
    ...
    _out_color = vec4(float(_TMP25.x), float(_TMP25.y), float(_TMP25.z), float((float((_input_dummy_rgb.x + _input_dummy_rgb.y + _input_dummy_rgb.z))/grey_balance + float((float(_is_on_dot)*baseline_alpha)))));
