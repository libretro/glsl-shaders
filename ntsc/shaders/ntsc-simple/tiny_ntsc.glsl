...
float sum = 0.0;
int N = 2; 
float timer = 0.0; if (InputSize.x < 300.0) timer = mod(float(FrameCount/2),2.0);
...
if (ln_delay == 1.0) line_delay = vTexCoord.y*SourceSize.y; // snes line delay
//snes line delay deployed at 170/256, 2/3 of pi(180'), 120' or 1/3 lines as in real snes
float phase = (vTexCoord.x*SourceSize.x + n - line_delay +timer )*PI*NTSC_RES/InputSize.x;
// comb filter line
...
float chroma = line - luma/2.0*comb;
final.r += luma*w;
